...
s.run_simulation(tmax=15, numpoints=75, sim='sphere')
s.post_process()
s.save_results('collapse3_sphere_sigma10_sim.mat')

# Large regularization
s.sigma = 0.25
s.run_simulation(tmax=15, numpoints=75, sim='sphere')
s.post_process()
s.save_results('collapse3_sphere_sigma25_sim.mat')

# Runge-Kutta, medium regularization
s.sigma = 0.10
s.run_simulation(tmax=15, numpoints=75, sim='rk4')
s.post_process()
s.save_results('collapse3_rk4_sigma10_sim.mat')

# Output for plotting 
s.sigma = 0.10
s.run_simulation(tmax=10, numpoints=1000, h=0.01, sim='sphere')
s.post_process()
s.save_results('collapse3_sphere_sigma10_hi_sim.mat')
